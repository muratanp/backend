#!/usr/bin/env python3
"""
Fix duplicate addresses in pnodes_status before creating unique index.
...
db = client[MONGO_DB]
pnodes_status = db["pnodes_status"]

print(f"Connected to database: {MONGO_DB}")
print(f"Collection: pnodes_status")
print()
...
duplicates = list(pnodes_status.aggregate(pipeline))
...
for dup in duplicates:
    ...
    for doc in docs[1:]:
        ...
        try:
            pnodes_status.delete_one({"_id": doc_id})
            deleted_count += 1
...
remaining_dups = list(pnodes_status.aggregate(pipeline))
...
try:
    ...
    try:
        pnodes_status.drop_index("address_1")
...
print("SUMMARY")
print("=" * 60)
total_docs = pnodes_status.count_documents({})
